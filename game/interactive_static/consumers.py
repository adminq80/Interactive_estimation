...
from .models import InteractiveStatic, InteractiveStaticRound, Settings, Task
from .message_executer import DelayedMessageExecutor
...
def get_round(game, user=None):
    ...
    for user in users.all():
        ...
        i_round, _ = InteractiveStaticRound.objects.get_or_create(user=user, game=game, plot=plot,
...
def lobby(message):
    ...
    games = InteractiveStatic.objects.filter(started=False).annotate(
...
def data_broadcast(message):
    ...
    if slider >= 0:
        ...
        if state == 'interactive':
            ...
            rounds = InteractiveStaticRound.objects.filter(
...
def interactive(user, game, round_data):
    current_round = InteractiveStaticRound.objects.get(
        user=user, game=game, round_order=round_data.get('current_round'))

    following = [{'username': u.username, 'avatar': u.get_avatar, 'guess': InteractiveStaticRound.objects.get(
        user=u,
...
def game_watcher(message):
    ...
    if user.prompted < game.constraints.max_prompts:
        ...
        game.user_send(user, action='logout', url=reverse('static_mode:exit'))
